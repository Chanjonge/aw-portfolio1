...
export type HomePagePreset = {
    /**
     * 특정 카테고리만 노출/선택(예: 컬렉션 slug=6).
     * slug는 백엔드 응답이 number/string 섞여도 안전하게 비교하기 위해 number 기준으로 받습니다.
     */
    lockedCategorySlug?: number;
    lockedCategoryName?: string;

    /** 초기 자동 선택 카테고리명(기존 메인: 독채형) */
    autoSelectCategoryName?: string;

    /** "전체" 버튼 숨김(locked 페이지는 true 권장) */
    hideAllButton?: boolean;

    /** lockedCategorySlug/Name을 찾았으면 해당 카테고리만 노출 */
...
export default function HomePageClient({
            ...
            (res) => {
                ...
                if (preset?.lockedCategorySlug != null || preset?.lockedCategoryName) {
                    ...
                    if (locked) {
                        ...
                        setSelectedCategory(locked.id);
                    } else {
                        // 못 찾으면 노출할 카테고리가 없으므로 빈 상태로 둡니다.
                        setCategories([]);
    ...
    const fetchPortfolios = useCallback(async () => {
        // locked 페이지는 selectedCategory가 정해지기 전 "전체(null)" 호출을 막습니다.
        if (preset?.requireSelectedCategory && !selectedCategory) {
            if (categoriesLoaded && isInitialLoad) {
                setLoading(false);
    ...
    }, [selectedCategory, isInitialLoad, preset?.requireSelectedCategory, categoriesLoaded]);
                        ...
                        {slides.map((slide, idx) => (
                            <SwiperSlide key={idx} className="relative">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 bg-black/30">
                                    <h3 className="text-white text-xl md:text-4xl lg:text-4xl line-height-1.5 font-bold mb-3 md:mb-4">{slide.title}</h3>
                                    <p className="text-white text-sm md:text-lg">{slide.subtitle}</p>
                                </div>
                            </SwiperSlide>
                        ))}
                    </Swiper>
