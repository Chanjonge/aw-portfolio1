...
export default function DynamicFormField({ question, value, onChange, error, disabled }: DynamicFormFieldProps) {
                    ...
                    <div className="flex items-center gap-1 text-lg font-semibold text-black">
                        ...
                        {question.thumbnail && (
                            <div className="relative inline-flex items-center gap-1">
                                ...
                                {showPreview && (
                                    <div ref={previewRef} className="absolute top-[0.5rem] md:top-[-10.5rem] left-[-8rem] md:left-[4rem] z-50 w-[11rem] md:w-[22rem] border border-gray-300 shadow-lg bg-white rounded-lg p-2">
    ...
    if (questionType === 'multi_text') {
        const isNumber = (parsedOptions as any)?.isNumber === true;
        const isIdPassword = (parsedOptions as any)?.isIdPassword === true;
        const [showPassword, setShowPassword] = useState(false);

        const placeholders = {
            placeholder1: (parsedOptions as any)?.placeholder1 || (isIdPassword ? '아이디를 입력하세요' : `${question.title} 을(를) 입력하세요`),
            placeholder2: (parsedOptions as any)?.placeholder2 || (isIdPassword ? '비밀번호를 입력하세요' : `${question.title} 을(를) 입력하세요`),
        };
        ...
        return (
            <div className="space-y-3">
                ...
                <input
                    ...
                    onChange={(e) => {
                        ...
                        if (isNumber) {
                            // 숫자만 허용
                            filtered = filtered.replace(/[^0-9]/g, '');
                        } else if (isIdPassword) {
                            // 아이디: 한글 제거
                            filtered = filtered.replace(/[ㄱ-ㅎㅏ-ㅣ가-힣]/g, '');
                        }
                ...
                <div className="relative">
                    <input
                        type={isIdPassword && !showPassword ? 'password' : 'text'}
                        inputMode={isNumber ? 'numeric' : 'text'}
                        value={values[1]}
                        onChange={(e) => {
                            let filtered = e.target.value;
                        ...
                        className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-black transition-all ${isIdPassword ? 'pr-12' : ''} ${error ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
                        placeholder={placeholders.placeholder2}
                        maxLength={question.maxLength}
                        disabled={disabled}
                    />
                    {isIdPassword && (
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-black transition-colors" title={showPassword ? '비밀번호 숨기기' : '비밀번호 보기'}>
                            {showPassword ? (
        ...
        return (
          ...
          ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer border-gray-300 hover:border-black'}`}
                ...
                {existingFiles.map((file: any, idx: number) => (
                    <div key={file.fileId ?? `temp-${idx}`} className="flex justify-between items-center gap-2">
                        {file.__temp ? (
                            <span className="text-sm text-gray-600">{file.name}</span>
                        ) : (
                            <button type="button" onClick={() => downloadFile(file)} className="underline text-sm text-left">
                                {file.name}
                            </button>
                        )}
                        {!disabled && (
                            <button type="button" onClick={() => (file.__temp ? onChange({ removeTempFileIndex: idx }) : onChange({ deleteFileId: file.fileId }))} className="text-xs text-red-500">
                                삭제
                            </button>
                        )}
                    </div>
        ...
        return (
            <div className="space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full">
                    <input value={zonecode} readOnly className="w-24 px-3 py-3 border-1 rounded-lg border-gray-300 bg-gray-100" placeholder="우편번호" disabled={disabled} />
                    <input value={fullAddress} readOnly className="flex-1 px-4 py-3 border-1 rounded-lg border-gray-300 bg-gray-100" placeholder="주소" disabled={disabled} />
                    <button type="button" onClick={handleSearchAddress} className="w-16 px-4 py-2 bg-black text-white rounded-lg text-sm font-semibold hover:bg-gray-800 transition-all" disabled={disabled}>
                        검색
                    </button>
                </div>
