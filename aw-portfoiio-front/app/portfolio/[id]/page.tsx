        ...
        if (question.questionType === "parlor") {
          ...
          const hasInvalidRoom = rooms.some((room) => {
            if (
              !room.name?.trim() ||
              !room.desc?.trim() ||
              !room.capacity?.standard?.trim() ||
              !room.capacity?.max?.trim() ||
              !room.type?.trim()
            )
              return true;
          ...
          try {
            ...
            if (isMultiple) {
              ...
              if (options.checkboxes && Array.isArray(options.checkboxes)) {
                ...
                const selectedOption = options.checkboxes.find(
                  (opt: any) => opt.label === selectedLabel,
                );
      ...
      setFormData((prev) => {
        ...
        return {
          ...
          [questionId]: prevValue.filter(
            (file: any) => file.fileId !== value.deleteFileId,
          ),
        };
      });
                              ...
                              {[
                                ...
                                <div key={key}>
                                  ...
                                  <div className="grid grid-cols-4 gap-4 mb-4">
                                    {["weekday", "fri", "sat", "sun"].map(
                                      (dayKey) => {
                                        const rawValue = (room as any)[key][
                                          dayKey
                                        ];
                                        // 천 단위 콤마 포맷팅 함수
                                        const formatNumber = (
                        ...
                        {specials.map((sp, index) => (
                          ...
                          >
                            ...
                            <div>
                              ...
                              />
                              <p
                                className={`text-xs mt-1 text-right ${sp.desc.length < 20 ? "text-red-500" : "text-gray-500"}`}
                              >
